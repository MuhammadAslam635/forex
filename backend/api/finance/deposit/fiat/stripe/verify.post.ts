...
export const metadata: OperationObject = {
  ...
  description:
    "Confirms the validity of a Stripe checkout session by its session ID, ensuring the session is authenticated and retrieving associated payment intent and line items details. Optionally, if the query parameter 'intent' is set to 'true', it uses the provided payment intent ID for confirmation.",
  ...
  parameters: [
    ...
    },
    {
      ...
      in: "query",
      description:
        "Set to 'true' to use provided payment intent ID for confirmation",
      required: false,
      schema: { type: "string" },
    },
    {
      ...
      name: "intentId",
      in: "query",
      description: "Payment intent ID to be used if the intent flag is 'true'",
      required: false,
      schema: { type: "string" },
    },
  ],
...
export default async (data: Handler) => {
  ...
  const { sessionId, intent, intentId } = query;
  ...
  try {
    ...
    // Determine how to retrieve the Payment Intent:
    let paymentIntent;
    if (intent === "true") {
      if (!intentId) {
        throw new Error("intentId is required when intent flag is 'true'");
      }
